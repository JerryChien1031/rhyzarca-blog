...
## Git-managed Blogger Pages

`_pages/about.md` 是 About 內容的唯一來源。修改後由既有 workflow 使用
Blogger **Pages** API 同步；不需登入後台建立 Page 或手填 ID。
`page_key` 是穩定識別，不隨顯示標題修改。`blogger_page_id`、`blogger_page_url`
初始留空，程式會以 API response 原子回寫，bot commit 保留 `[skip ci]`。

Resolver 列出包含 LIVE、DRAFT、SOFT_TRASHED 的全部分頁，交叉檢查已存 ID、
URL path 和內容中的 `data-rhyzarca-page-key`，最後才以標題復原。
多重匹配、不同 key、已刪除頁或 API 錯誤一律停止，不盲目 insert。
既有草稿可由 update 的 `publish=True` 自動發布。

首次建立先以 `page_key`（About 為 `about`）當標題，再更新為顯示標題。
Blogger 沒有 documented custom slug 欄位；**不能保證服務端一定配置指定 URL**。
程式檢查真實 response 是否為 `/p/about.html`，若不符，仍保存真實 ID / URL，
讓 workflow 失敗並在重跑時更新同一頁，不以假 URL、刪頁重建或人工後台流程掩蓋。
本次唯讀 OAuth 檢查顯示沒有現有 Pages；正式建立因自動核准審查阻擋，尚未驗證 slug。

Workflow concurrency 保證此 workflow 串行；請勿同時啟動本機寫入同步。
Blogger 沒有唯一 key / 原子 upsert，無法對外部並行寫入或服務端讀取延遲提供絕對去重保證。
已建立但回寫中斷時，可由內容 key、URL 或初始標題找回，不自動重試 insert。
即使同步失敗，workflow 仍提交已取得的 Markdown identity；Secrets 僅由忽略檔案載入。

首頁實際「關於我」來自 `.widget.Profile#Profile1`。
Git 管理的 JS 改寫該 Gadget 與作者 byline 的本站作者連結；不更動讀者留言連結。
About 注入 ProfilePage、文章注入 TechArticle，兩者使用相同 Person ID。
Theme 原有的本站作者 BlogPosting 在非文章頁移除，文章頁則修正 author URL。
這些是 rendered DOM 修正；Theme 的 server-side HTML 並未透過 repository 改動。
...
- https://developers.google.com/blogger/docs/3.0/reference/pages/insert
